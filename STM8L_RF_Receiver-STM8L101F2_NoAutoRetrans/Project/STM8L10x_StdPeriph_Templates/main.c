...
void main(void)
{
  ...
  CE_HIGH();  // Set Chip Enable (CE) pin high to enable reciever
              // goes to RX mode after settling time of 130us
  ...
  while (1)
  {
    switch(state)
    {
      ...
      case ST_WAIT_INPUT_OR_FLAG:
      {
        ...
        if (!NRF_IRQ_STATE)                         // Check if an interupt is
        {
          // NRF24 interrupt request meaning that the first dta packet arrived
          Timeout_SetTimeout2(20);  // wait here for the other two data packets to be stored in the RX FIFO
          while(!Timeout_IsTimeout2());
          
          radio_irq ();  // read RX FIFO and flush it
          if ((radio_get_status ()) == RF_RX_DR)
          {
            if(Timeout_IsTimeout3())
            {
              // Set radio status to idle
              radio_set_status(RF_IDLE);
              Timeout_SetTimeout3(1200);  // set timeout for two consecutive user commands
              state = ST_NRF_RCVD_DATA;
            }
            else
            {
              // do nothing  
      ...
      case ST_NRF_RCVD_DATA:
      {
        BtoDW.B.b0 = radio_get_pload_byte(0);  // convert 4 bytes to u32
        BtoDW.B.b1 = radio_get_pload_byte(1);
        BtoDW.B.b2 = radio_get_pload_byte(2);
        BtoDW.B.b3 = radio_get_pload_byte(3);

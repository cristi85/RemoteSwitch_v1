 ...
 157  0005 4f            	clr	a
 158  0006 cd0000        	call	_Errors_ResetError
 160                     ; 40   nrf24_configRegister(RF_CH,channel);
 162  0009 7b03          	ld	a,(OFST+1,sp)
 163  000b 97            	ld	xl,a
 164  000c a605          	ld	a,#5
 165  000e 95            	ld	xh,a
 166  000f cd0000        	call	_nrf24_configRegister
 168                     ; 41   nrf24_readRegister(RF_CH, &rcv, 1);
 170  0012 4b01          	push	#1
 171  0014 96            	ldw	x,sp
 172  0015 1c0003        	addw	x,#OFST+1
 173  0018 89            	pushw	x
 174  0019 a605          	ld	a,#5
 175  001b cd0000        	call	_nrf24_readRegister
 177  001e 5b03          	addw	sp,#3
 178                     ; 42   if(rcv != channel)
 180  0020 7b02          	ld	a,(OFST+0,sp)
 181  0022 1103          	cp	a,(OFST+1,sp)
 182  0024 2704          	jreq	L35
 183                     ; 44     Errors_SetError(ERROR_NRF24_INIT);
 185  0026 4f            	clr	a
 186  0027 cd0000        	call	_Errors_SetError
 188  002a               L35:
 189                     ; 48   nrf24_configRegister(RX_PW_P0, 0x00); // Auto-ACK pipe ...
 191  002a ae1100        	ldw	x,#4352
 192  002d cd0000        	call	_nrf24_configRegister
 194                     ; 49   nrf24_readRegister(RX_PW_P0, &rcv, 1);
 196  0030 4b01          	push	#1
 197  0032 96            	ldw	x,sp
 198  0033 1c0003        	addw	x,#OFST+1
 199  0036 89            	pushw	x
 200  0037 a611          	ld	a,#17
 201  0039 cd0000        	call	_nrf24_readRegister
 203  003c 5b03          	addw	sp,#3
 204                     ; 50   if(rcv != 0x00)
 206  003e 7b02          	ld	a,(OFST+0,sp)
 207  0040 2704          	jreq	L55
 208                     ; 52     Errors_SetError(ERROR_NRF24_INIT);
 210  0042 4f            	clr	a
 211  0043 cd0000        	call	_Errors_SetError
 213  0046               L55:
 214                     ; 55   nrf24_configRegister(RX_PW_P1, payload_len); // Data payload pipe
 216  0046 b600          	ld	a,_payload_len
 217  0048 97            	ld	xl,a
 218  0049 a612          	ld	a,#18
 219  004b 95            	ld	xh,a
 220  004c cd0000        	call	_nrf24_configRegister
 222                     ; 56   nrf24_readRegister(RX_PW_P1, &rcv, 1);
 224  004f 4b01          	push	#1
 225  0051 96            	ldw	x,sp
 226  0052 1c0003        	addw	x,#OFST+1
 227  0055 89            	pushw	x
 228  0056 a612          	ld	a,#18
 229  0058 cd0000        	call	_nrf24_readRegister
 231  005b 5b03          	addw	sp,#3
 232                     ; 57   if(rcv != payload_len)
 234  005d 7b02          	ld	a,(OFST+0,sp)
 235  005f b100          	cp	a,_payload_len
 236  0061 2704          	jreq	L75
 237                     ; 59     Errors_SetError(ERROR_NRF24_INIT);
 239  0063 4f            	clr	a
 240  0064 cd0000        	call	_Errors_SetError
 242  0067               L75:
 243                     ; 62   nrf24_configRegister(RX_PW_P2, 0x00); // Pipe not used 
 245  0067 ae1300        	ldw	x,#4864
 246  006a cd0000        	call	_nrf24_configRegister
 248                     ; 63   nrf24_readRegister(RX_PW_P2, &rcv, 1);
 250  006d 4b01          	push	#1
 251  006f 96            	ldw	x,sp
 252  0070 1c0003        	addw	x,#OFST+1
 253  0073 89            	pushw	x
 254  0074 a613          	ld	a,#19
 255  0076 cd0000        	call	_nrf24_readRegister
 257  0079 5b03          	addw	sp,#3
 258                     ; 64   if(rcv != 0x00)
 260  007b 7b02          	ld	a,(OFST+0,sp)
 261  007d 2704          	jreq	L16
 262                     ; 66     Errors_SetError(ERROR_NRF24_INIT);
 264  007f 4f            	clr	a
 265  0080 cd0000        	call	_Errors_SetError
 267  0083               L16:
 268                     ; 69   nrf24_configRegister(RX_PW_P3, 0x00); // Pipe not used 
 270  0083 ae1400        	ldw	x,#5120
 271  0086 cd0000        	call	_nrf24_configRegister
 273                     ; 70   nrf24_readRegister(RX_PW_P3, &rcv, 1);
 275  0089 4b01          	push	#1
 276  008b 96            	ldw	x,sp
 277  008c 1c0003        	addw	x,#OFST+1
 278  008f 89            	pushw	x
 279  0090 a614          	ld	a,#20
 280  0092 cd0000        	call	_nrf24_readRegister
 282  0095 5b03          	addw	sp,#3
 283                     ; 71   if(rcv != 0x00)
 285  0097 7b02          	ld	a,(OFST+0,sp)
 286  0099 2704          	jreq	L36
 287                     ; 73     Errors_SetError(ERROR_NRF24_INIT);
 289  009b 4f            	clr	a
 290  009c cd0000        	call	_Errors_SetError
 292  009f               L36:
 293                     ; 76   nrf24_configRegister(RX_PW_P4, 0x00); // Pipe not used 
 295  009f ae1500        	ldw	x,#5376
 296  00a2 cd0000        	call	_nrf24_configRegister
 298                     ; 77   nrf24_readRegister(RX_PW_P4, &rcv, 1);
 300  00a5 4b01          	push	#1
 301  00a7 96            	ldw	x,sp
 302  00a8 1c0003        	addw	x,#OFST+1
 303  00ab 89            	pushw	x
 304  00ac a615          	ld	a,#21
 305  00ae cd0000        	call	_nrf24_readRegister
 307  00b1 5b03          	addw	sp,#3
 308                     ; 78   if(rcv != 0x00)
 310  00b3 7b02          	ld	a,(OFST+0,sp)
 311  00b5 2704          	jreq	L56
 312                     ; 80     Errors_SetError(ERROR_NRF24_INIT);
 314  00b7 4f            	clr	a
 315  00b8 cd0000        	call	_Errors_SetError
 317  00bb               L56:
 318                     ; 83   nrf24_configRegister(RX_PW_P5, 0x00); // Pipe not used 
 320  00bb ae1600        	ldw	x,#5632
 321  00be cd0000        	call	_nrf24_configRegister
 323                     ; 84   nrf24_readRegister(RX_PW_P5, &rcv, 1);
 325  00c1 4b01          	push	#1
 326  00c3 96            	ldw	x,sp
 327  00c4 1c0003        	addw	x,#OFST+1
 328  00c7 89            	pushw	x
 329  00c8 a616          	ld	a,#22
 330  00ca cd0000        	call	_nrf24_readRegister
 332  00cd 5b03          	addw	sp,#3
 333                     ; 85   if(rcv != 0x00)
 335  00cf 7b02          	ld	a,(OFST+0,sp)
 336  00d1 2704          	jreq	L76
 337                     ; 87     Errors_SetError(ERROR_NRF24_INIT);
 339  00d3 4f            	clr	a
 340  00d4 cd0000        	call	_Errors_SetError
 342  00d7               L76:
 343                     ; 92   send = (1<<RF_DR_LOW)|((3)<<RF_PWR);
 345  00d7 a626          	ld	a,#38
 346  00d9 6b01          	ld	(OFST-1,sp),a
 347                     ; 93   nrf24_configRegister(RF_SETUP, send);
 349  00db ae0626        	ldw	x,#1574
 350  00de cd0000        	call	_nrf24_configRegister
 352                     ; 94   nrf24_readRegister(RF_SETUP, &rcv, 1);
 354  00e1 4b01          	push	#1
 355  00e3 96            	ldw	x,sp
 356  00e4 1c0003        	addw	x,#OFST+1
 357  00e7 89            	pushw	x
 358  00e8 a606          	ld	a,#6
 359  00ea cd0000        	call	_nrf24_readRegister
 361  00ed 5b03          	addw	sp,#3
 362                     ; 95   if(rcv != send)
 364  00ef 7b02          	ld	a,(OFST+0,sp)
 365  00f1 1101          	cp	a,(OFST-1,sp)
 366  00f3 2704          	jreq	L17
 367                     ; 97     Errors_SetError(ERROR_NRF24_INIT);
 369  00f5 4f            	clr	a
 370  00f6 cd0000        	call	_Errors_SetError
 372  00f9               L17:
 373                     ; 101   send = nrf24_CONFIG;
 375  00f9 a638          	ld	a,#56
 376  00fb 6b01          	ld	(OFST-1,sp),a
 377                     ; 102   nrf24_configRegister(CONFIG, send);
 379  00fd ae0038        	ldw	x,#56
 380  0100 cd0000        	call	_nrf24_configRegister
 382                     ; 103   nrf24_readRegister(CONFIG, &rcv, 1);
 384  0103 4b01          	push	#1
 385  0105 96            	ldw	x,sp
 386  0106 1c0003        	addw	x,#OFST+1
 387  0109 89            	pushw	x
 388  010a 4f            	clr	a
 389  010b cd0000        	call	_nrf24_readRegister
 391  010e 5b03          	addw	sp,#3
 392                     ; 104   if(rcv != send)
 394  0110 7b02          	ld	a,(OFST+0,sp)
 395  0112 1101          	cp	a,(OFST-1,sp)
 396  0114 2704          	jreq	L37
 397                     ; 106     Errors_SetError(ERROR_NRF24_INIT);
 399  0116 4f            	clr	a
 400  0117 cd0000        	call	_Errors_SetError
 402  011a               L37:
 403                     ; 110   send = (1<<ENAA_P0)|(1<<ENAA_P1)|(0<<ENAA_P2)|(0<<ENAA_P3)|(0<<ENAA_P4)|(0<<ENAA_P5);
 405  011a a603          	ld	a,#3
 406  011c 6b01          	ld	(OFST-1,sp),a
 407                     ; 111   nrf24_configRegister(EN_AA, send);
 409  011e ae0103        	ldw	x,#259
 410  0121 cd0000        	call	_nrf24_configRegister
 412                     ; 112   nrf24_readRegister(EN_AA, &rcv, 1);
 414  0124 4b01          	push	#1
 415  0126 96            	ldw	x,sp
 416  0127 1c0003        	addw	x,#OFST+1
 417  012a 89            	pushw	x
 418  012b a601          	ld	a,#1
 419  012d cd0000        	call	_nrf24_readRegister
 421  0130 5b03          	addw	sp,#3
 422                     ; 113   if(rcv != send)
 424  0132 7b02          	ld	a,(OFST+0,sp)
 425  0134 1101          	cp	a,(OFST-1,sp)
 426  0136 2704          	jreq	L57
 427                     ; 115     Errors_SetError(ERROR_NRF24_INIT);
 429  0138 4f            	clr	a
 430  0139 cd0000        	call	_Errors_SetError
 432  013c               L57:
 433                     ; 119   send = (1<<ERX_P0)|(1<<ERX_P1)|(0<<ERX_P2)|(0<<ERX_P3)|(0<<ERX_P4)|(0<<ERX_P5);
 435  013c a603          	ld	a,#3
 436  013e 6b01          	ld	(OFST-1,sp),a
 437                     ; 120   nrf24_configRegister(EN_RXADDR, send);
 439  0140 ae0203        	ldw	x,#515
 440  0143 cd0000        	call	_nrf24_configRegister
 442                     ; 121   nrf24_readRegister(EN_RXADDR, &rcv, 1);
 444  0146 4b01          	push	#1
 445  0148 96            	ldw	x,sp
 446  0149 1c0003        	addw	x,#OFST+1
 447  014c 89            	pushw	x
 448  014d a602          	ld	a,#2
 449  014f cd0000        	call	_nrf24_readRegister
 451  0152 5b03          	addw	sp,#3
 452                     ; 122   if(rcv != send)
 454  0154 7b02          	ld	a,(OFST+0,sp)
 455  0156 1101          	cp	a,(OFST-1,sp)
 456  0158 2704          	jreq	L77
 457                     ; 124     Errors_SetError(ERROR_NRF24_INIT);
 459  015a 4f            	clr	a
 460  015b cd0000        	call	_Errors_SetError
 462  015e               L77:
 463                     ; 128   send = (0x04<<ARD)|(0x0F<<ARC);
 465  015e a64f          	ld	a,#79
 466  0160 6b01          	ld	(OFST-1,sp),a
 467                     ; 129   nrf24_configRegister(SETUP_RETR, send);
 469  0162 ae044f        	ldw	x,#1103
 470  0165 cd0000        	call	_nrf24_configRegister
 472                     ; 130   nrf24_readRegister(SETUP_RETR, &rcv, 1);
 474  0168 4b01          	push	#1
 475  016a 96            	ldw	x,sp
 476  016b 1c0003        	addw	x,#OFST+1
 477  016e 89            	pushw	x
 478  016f a604          	ld	a,#4
 479  0171 cd0000        	call	_nrf24_readRegister
 481  0174 5b03          	addw	sp,#3
 482                     ; 131   if(rcv != send)
 484  0176 7b02          	ld	a,(OFST+0,sp)
 485  0178 1101          	cp	a,(OFST-1,sp)
 486  017a 2704          	jreq	L101
 487                     ; 133     Errors_SetError(ERROR_NRF24_INIT);
 489  017c 4f            	clr	a
 490  017d cd0000        	call	_Errors_SetError
 492  0180               L101:
 493                     ; 137   send = (0<<DPL_P0)|(0<<DPL_P1)|(0<<DPL_P2)|(0<<DPL_P3)|(0<<DPL_P4)|(0<<DPL_P5);
 495  0180 0f01          	clr	(OFST-1,sp)
 496                     ; 138   nrf24_configRegister(DYNPD, send);
 498  0182 ae1c00        	ldw	x,#7168
 499  0185 cd0000        	call	_nrf24_configRegister
 501                     ; 139   nrf24_readRegister(DYNPD, &rcv, 1);
 503  0188 4b01          	push	#1
 504  018a 96            	ldw	x,sp
 505  018b 1c0003        	addw	x,#OFST+1
 506  018e 89            	pushw	x
 507  018f a61c          	ld	a,#28
 508  0191 cd0000        	call	_nrf24_readRegister
 510  0194 5b03          	addw	sp,#3
 511                     ; 140   if(rcv != send)
 513  0196 7b02          	ld	a,(OFST+0,sp)
 514  0198 1101          	cp	a,(OFST-1,sp)
 515  019a 2704          	jreq	L301
 516                     ; 142     Errors_SetError(ERROR_NRF24_INIT);
 518  019c 4f            	clr	a
 519  019d cd0000        	call	_Errors_SetError
 521  01a0               L301:
 522                     ; 146   nrf24_powerUpRx();
 524  01a0 cd0000        	call	_nrf24_powerUpRx
 526                     ; 147 }
 529  01a3 5b04          	addw	sp,#4
 530  01a5 81            	ret	
...
1512  002b 2704          	jreq	L714
1513                     ; 344       Errors_SetError(ERROR_NRF24_INIT);
1515  002d 4f            	clr	a
1516  002e cd0000        	call	_Errors_SetError
1518  0031               L714:
1519                     ; 347     nrf24_ce_digitalWrite(LOW);    
1521  0031 4f            	clr	a
1522  0032 cd0000        	call	_nrf24_ce_digitalWrite
1524                     ; 348     send = nrf24_CONFIG|((1<<PWR_UP)|(1<<PRIM_RX));
1526  0035 a63b          	ld	a,#59
1527  0037 6b02          	ld	(OFST+0,sp),a
1528                     ; 349     nrf24_configRegister(CONFIG, send);
1530  0039 ae003b        	ldw	x,#59
1531  003c cd0000        	call	_nrf24_configRegister
1533                     ; 350     nrf24_readRegister(CONFIG, &rcv, 1);
1535  003f 4b01          	push	#1
1536  0041 96            	ldw	x,sp
1537  0042 1c0002        	addw	x,#OFST+0
1538  0045 89            	pushw	x
1539  0046 4f            	clr	a
1540  0047 cd0000        	call	_nrf24_readRegister
1542  004a 5b03          	addw	sp,#3
1543                     ; 351     if(rcv != send)
1545  004c 7b01          	ld	a,(OFST-1,sp)
1546  004e 1102          	cp	a,(OFST+0,sp)
1547  0050 2704          	jreq	L124
1548                     ; 353       Errors_SetError(ERROR_NRF24_INIT);
1550  0052 4f            	clr	a
1551  0053 cd0000        	call	_Errors_SetError
1553  0056               L124:
1554                     ; 355     nrf24_ce_digitalWrite(HIGH);
1556  0056 a601          	ld	a,#1
1557  0058 cd0000        	call	_nrf24_ce_digitalWrite
1559                     ; 356 }
1562  005b 85            	popw	x
1563  005c 81            	ret	

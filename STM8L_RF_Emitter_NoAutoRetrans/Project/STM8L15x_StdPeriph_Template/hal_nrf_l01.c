...
void hal_nrf_set_datarate(hal_nrf_datarate_t datarate)
{
  switch(datarate)
  {
    ...
    {
      hal_nrf_write_reg(RF_SETUP, ( (hal_nrf_read_reg(RF_SETUP)|(1<<RF_DR_LOW)) & ~(1<< RF_DR_HIGH) ));
      break;  
    }
    case HAL_NRF_1MBPS:
    {
      hal_nrf_write_reg(RF_SETUP, ( hal_nrf_read_reg(RF_SETUP) & ~(1<<RF_DR_HIGH | 1<<RF_DR_LOW) ));
      break;  
    }
    case HAL_NRF_2MBPS:
    {
...
{
  uint8_t tmp = hal_nrf_read_reg(RF_SETUP);
  return ((tmp & (1<<RF_DR_HIGH)) >> (RF_DR_HIGH-1)) | ((tmp & (1<<RF_DR_LOW)) >> (RF_DR_LOW));
}
